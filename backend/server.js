...
// Compare Car
// Route to fetch two cars by their IDs
app.get("/carinfo/:carId", async (req, res) => {
  const { carId } = req.params; // Get the car ID from the URL parameter
...
app.get("/carinfo/:carId1/:carId2", async (req, res) => {
  const { carId1, carId2 } = req.params; // Get the car IDs from the URL parameters

  try {
    // Fetch two cars by their IDs
